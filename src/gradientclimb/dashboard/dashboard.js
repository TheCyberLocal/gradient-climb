...
function overview(data) {
  ...
  return stats + `<div class="grid two">${panel("Quality over experience", "Observed points for each run; gaps are not imputed.", curves, "Run comparison")}${panel("Latest held-out results", "Separate evidence by environment and evaluation protocol.", evidence, `${data.evaluations.length} records`)}</div>` + panel("Recent experiments", "Select a run to inspect its complete provenance and artifact manifest.", runTable(data.runs,8), `${data.runs.length} in scope`);
}
...
function controls(data) {
  ...
  const trace=[{name:"Gas",color:palette[0],points:rows.flatMap(row=>[{x:row.elapsed_seconds,y:row.gas?1:0},{x:row.elapsed_seconds+row.action_duration_seconds,y:row.gas?1:0}])},{name:"Brake / reverse",color:palette[2],points:rows.flatMap(row=>[{x:row.elapsed_seconds,y:row.brake?1:0},{x:row.elapsed_seconds+row.action_duration_seconds,y:row.brake?1:0}])}];
  const traces=rows.length?runSelect+chart(trace,{zero:true,stepped:true,yLabel:"Pedal state"})+table(["Episode","Step","Elapsed","Gas","Brake","Duration"],rows.slice(0,100).map(row=>[esc(row.episode_id),esc(row.step),seconds(row.elapsed_seconds),row.gas?"1":"0",row.brake?"1":"0",seconds(row.action_duration_seconds)])):empty("Temporal pedal traces not measured","All four states (00, 10, 01, 11) are represented in the schema. Episode action counts alone do not reconstruct action duration.");
