...
A later same-sized native paused capture was rejected as unknown by these frozen prototypes (PAUSED similarity 0.8813 versus threshold 0.98). That separate one-image check is recorded in [discovery labeling](discovery-labeling.md); it demonstrates that matching dimensions do not establish template transfer. [Offline pixel measurement](pixel-measurements.md) now supplies wheel/body/material-boundary features and conservative camera diagnostics, with real source-run coverage and synthetic tests stated separately.
